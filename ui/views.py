...
class GridView(SheetView):
    ...
    def _cell_style(self, f: _Frame, rank, cell=None):
        ...
        #
        #   ★ 上一轮我改错了地方 ★
        #     那句话说的是**所有**预告格，我只改了训练面板那边，
        #     于是两个面板长得完全不一样 —— 用户一眼就看出来了。
        #
        #   ★ 为什么可以不要淡黄 ★
        #     淡黄那套的用途是"用颜色深浅读先后"（`T.UPCOMING` 从亮到淡）。
        #     可现在"这是第几个"已经有**序号角标** `1 2 3…` 在说了
        #     （`_paint_badges` 画的，跟底色无关）—— 一份信息说两遍，
        #     而且浅黄那一大块在深色网格里特别抢眼，把"当前格"都压住了。
        #
        #   现在：**当前格（深橄榄/亮边）跟预告格（普通深色）分得清清楚楚**，
        #   后面几个在哪靠角标读。两边面板也终于是同一副面孔了。
        #   （`T.UPCOMING` 那五个渐淡色暂时没人用了，留着 ——
        #    它是"渐变色"的现成素材，哪天想给预告格加个淡淡的层次能直接用。）
        return T.CELL, T.CELL_EDGE, T.TEXT_DIM, 0.0
