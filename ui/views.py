...
class SheetView(QWidget):
    ...
    def set_flash(self, pitch: str, seconds: float = 0.35,
        ...
        ★ 同一个键在 `min_gap` 内重复触发会被忽略（默认 0.45 秒）★
          这一段是**麦克风识别时代**留下的：那会儿采样是 0.79 秒的衰减音，
          识别器在它衰减的过程中可能报好几次（各次谐波衰减速度不同，
          激活会起伏），用户看到的就是**「按一下，浮窗闪 2~3 下」**。
          试过在识别器层面治（去抖窗口、峰值跟踪、能量上升门限…），
          但那些手段**同时也会吃掉真正的快速连奏**（同一个键 0.27 秒后
          再按一次）—— 两者在能量上是同一个现象，分不开。
          所以放到这一层：识别器保持灵敏，浮窗只负责别闪花眼。

          识别器后来整套砍掉了（闪烁改由打击垫点击**直接驱动**），
          但这个"别闪花眼"的闸门留着 —— 连点同一格的场景照样有，
          而且 `_flash_last` 是**每个键各自计时**，比当时更准。

          ★ 这行原来写的是「0.25 秒」★ 那是很早以前的值，
            参数默认值早就改成 0.45 了。文档跟代码对不上最坑 ——
            照着注释去调参的人会算出完全不对的结果。
...
# ★ 字体是按"字号 + 粗细"复用的 ★
#
#   审计实测（offscreen，30000 次平均）：`QFont()` + `setPointSizeF()` +
#   `setBold()` 一次 **1.44 µs**。而 `_paint_labels` 每格要两次
#   （音名 + 手指号）、`_paint_badges` 每格一次、`_paint_countdown`
#   每个数字一次 —— 满屏 16 格算下来一帧 50~70 个 QFont，
#   约 100 µs 纯花在"建对象"上。有收缩圈时浮窗是 8ms 重绘一次（125fps）。
#
#   `_fit_font()` 是**纯函数**：同样的 `(size, bold)` 一定得到同样的字体。
#   所以结果可以直接复用。
#
#   ★ key 用**原始 size**，不做 round ★
#     第一版想 `round(size, 1)` 来压缩条目数，但那等于**改字号** ——
#     `34.24` 会变成 `34.2`，画出来就不一样了。宁可多几条缓存，
#     也不能动画面。
#   ★ 条目数封顶 ★
#     用户拖「大小」滑块时 cell 一直在变，key 跟着变。封个顶，
#     超了就整个清掉 —— 反正下一帧重新攒，比无限涨好。
#   ★ QFont 是值类型 ★
#     `p.setFont(f)` 内部存的是副本，所以同一个 QFont 对象给多个
#     painter 用是安全的。
_FONT_CACHE: dict[tuple[float, bool], QFont] = {}
